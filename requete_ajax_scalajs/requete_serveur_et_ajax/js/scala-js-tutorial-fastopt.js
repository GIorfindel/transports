...
$c_Ltutorial_webapp_TutorialApp$.prototype.rechItin__Lorg_scalajs_jquery_JQueryEventObject__V = (function(e) {
  ...
  jsx$1.ajax({
    ...
    "success": (function(data$2, textStatus$2, jqXHR$2) {
      ...
      return (0, $m_Lorg_scalajs_jquery_package$().jQuery$1)("#iti").html($m_Ltutorial_webapp_TutorialApp$().afficheIti__sjs_js_Dynamic__T(json))
    }),
    "error": (function(jqXHR$2$1, textStatus$2$1, errorThrow$2) {
...
$c_Ltutorial_webapp_TutorialApp$.prototype.afficheIti__sjs_js_Dynamic__T = (function(iti) {
  if (($objectToString(iti.status) === "OK")) {
    var r = iti.routes;
    if ((!$uZ($m_sjs_js_WrappedDictionary$Cache$().safeHasOwnProperty$1.call(r, "0")))) {
      throw new $c_ju_NoSuchElementException().init___T("key not found: 0")
    };
    var route = r["0"];
    if ((!$uZ($m_sjs_js_WrappedDictionary$Cache$().safeHasOwnProperty$1.call(route, "legs")))) {
      throw new $c_ju_NoSuchElementException().init___T("key not found: legs")
    };
    var legs = route.legs;
    if ((!$uZ($m_sjs_js_WrappedDictionary$Cache$().safeHasOwnProperty$1.call(legs, "0")))) {
      throw new $c_ju_NoSuchElementException().init___T("key not found: 0")
    };
    var leg = legs["0"];
    if ((!$uZ($m_sjs_js_WrappedDictionary$Cache$().safeHasOwnProperty$1.call(leg, "start_address")))) {
      throw new $c_ju_NoSuchElementException().init___T("key not found: start_address")
    };
    var jsx$1 = leg.start_address;
    if ((!$uZ($m_sjs_js_WrappedDictionary$Cache$().safeHasOwnProperty$1.call(leg, "end_address")))) {
      throw new $c_ju_NoSuchElementException().init___T("key not found: end_address")
    };
    var elem = (((("<p>Depart:" + jsx$1) + ", Arrivee:") + leg.end_address) + "</p>");
    var elem$1 = null;
    elem$1 = elem;
    if ((!$uZ($m_sjs_js_WrappedDictionary$Cache$().safeHasOwnProperty$1.call(leg, "steps")))) {
      throw new $c_ju_NoSuchElementException().init___T("key not found: steps")
    };
    var steps = leg.steps;
    var array = $g.Object.keys(steps);
    var i = 0;
    ...
    while ((i < len)) {
      ...
      var k = $as_T(arg1);
      if ((!$uZ($m_sjs_js_WrappedDictionary$Cache$().safeHasOwnProperty$1.call(steps, k)))) {
        throw new $c_ju_NoSuchElementException().init___T(("key not found: " + k))
      };
      var step = steps[k];
      if ((!$uZ($m_sjs_js_WrappedDictionary$Cache$().safeHasOwnProperty$1.call(step, "travel_mode")))) {
        throw new $c_ju_NoSuchElementException().init___T("key not found: travel_mode")
      };
      if (($objectToString(step.travel_mode) === "TRANSIT")) {
        if ((!$uZ($m_sjs_js_WrappedDictionary$Cache$().safeHasOwnProperty$1.call(step, "transit_details")))) {
          throw new $c_ju_NoSuchElementException().init___T("key not found: transit_details")
        };
        var t = step.transit_details;
        elem$1 = ((((((($as_T(elem$1) + "<p>") + t.line.vehicle.name) + ", line:") + t.line.short_name) + " vers ") + t.headsign) + "</p>");
        elem$1 = ((((($as_T(elem$1) + "<p>--- arret ") + t.departure_stop.name) + ", a ") + t.departure_time.text) + "</p>");
        elem$1 = ((((($as_T(elem$1) + "<p>--- sortir ") + t.arrival_stop.name) + ", a ") + t.arrival_time.text) + "</p>")
      };
      i = ((1 + i) | 0)
    };
    return $as_T(elem$1)
